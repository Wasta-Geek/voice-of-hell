...
function ProfileKeyEffectItem({
  profileIndex,
  keybindIndex,
}: ProfileKeyEffectItemProps) {
  const [config, setConfig] = useConfig();
  const soundEffectTypeList = Object.entries(SoundEffectType).map(
    ([value, label]) => ({ label: label, value: value }),
  );

  if (!config) return null;

  const handleSelectSoundEffectType = useCallback(
    (value: string | null) => {
      ...
      if (config) {
        ...
        new_config.profiles[profileIndex].keybind_config[
          keybindIndex
        ].sound_effect = generateSoundEffect(value as RustSoundEffect);
  ...
  function KeyEffectSubElements({
    ...
    switch (
      config?.profiles[profileIndex].keybind_config[keybindIndex].sound_effect
        .type
    ) {
      case RustSoundEffect.IncreaseVolume:
      case RustSoundEffect.DecreaseVolume:
        return (
          <KeyEffectVolumeSlider
