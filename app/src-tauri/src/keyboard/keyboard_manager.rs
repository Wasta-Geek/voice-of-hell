...
fn poll(thread_should_run_bool: Arc<AtomicBool>, app_handle: AppHandle) {
    ...
    while thread_should_run_bool.load(Ordering::Relaxed) {
        ...
        let key_released_list: HashSet<_> = previous_key_pressed_list.difference(&key_pressed_list).into_iter().collect();
        let new_key_pressed_list: HashSet<_> = key_pressed_list.difference(&previous_key_pressed_list).into_iter().collect();
        ...
        if config.runtime.keybind_listening && (new_key_pressed_list.len() > 0 || key_released_list.len() > 0) {
            let emit_result = app_handle.emit(
                "keyboard_state_changed",
                KeyboardState {
                    key_pressed_list: Vec::from_iter(key_pressed_list.clone().into_iter()).vec_into(),
                    key_released_list: Vec::from_iter(key_released_list.clone().into_iter()).vec_into()
                },
            );
